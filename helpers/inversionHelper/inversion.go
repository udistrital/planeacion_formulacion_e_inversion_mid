...
func RegistrarProyecto(registroProyecto map[string]interface{}) map[string]interface{} {
	...
	if err := request.SendJson("http://"+beego.AppConfig.String("PlanesService")+"/plan", "POST", &respuestaProyecto, plan); err != nil {
...
func RegistrarInfoComplementaria(idProyecto string, infoProyecto map[string]interface{}, nombreCoplementaria string) error {
	...
	errSubgrupo := request.SendJson("http://"+beego.AppConfig.String("PlanesService")+"/subgrupo", "POST", &resSubgrupo, infoSubgrupo)
	if errSubgrupo == nil {
		...
		errDetalle := request.SendJson("http://"+beego.AppConfig.String("PlanesService")+"/subgrupo-detalle", "POST", &resDetalle, subgrupoDetalle)
		return errDetalle
	}
...
func ActualizarInfoComplDetalle(idSubgrupo string, detalleData []interface{}) error {
	...
	if errGet == nil {
		...
		var resDetalle map[string]interface{}
		errDetalle := request.SendJson("http://"+beego.AppConfig.String("PlanesService")+"/subgrupo-detalle/"+idSubgrupo, "PUT", &resDetalle, subgrupo)
		return errDetalle
	}
	return errGet
}
...
func ActualizarPresupuestoDisponible(infoFuente []interface{}) {
	for _, fuente := range infoFuente {
		...
		if err == nil {
			...
			request.SendJson("http://"+beego.AppConfig.String("PlanesService")+"/fuentes-apropiacion/"+fuente.(map[string]interface{})["id"].(string), "PUT", &dataFuente, resFuente)
		}
	}
}
...
// 		request.LimpiezaRespuestaRefactor(res[0], &infoSubgrupos)
				...
				for key := range dato_plan {
					...
					if armonizacion_dato[key] != nil {
						...
						if dataProyect["idSubDetalleProI"] != nil {
							...
							request.LimpiezaRespuestaRefactor(res, &respuestaLimpia2)
